...
//    Info << areaESIS_[1] << endl;
//    Info << areaESIS_[2] << endl;
//    Info << areaESIS_[3] << endl;
...
bool Foam::functionObjects::fobjArea::write()
{
    ...
    {
        fobjESISi = (flowESIS[i] / (flowTot * (areaESIS_[i] / areaTot)) - 1) 
                    / max((flowESIS[i] / (flowTot * (areaESIS_[i] / areaTot))), 1.0);
    ...
    {
        Info << endl;
        Info << "######## FObj convergence check ########" << endl;
